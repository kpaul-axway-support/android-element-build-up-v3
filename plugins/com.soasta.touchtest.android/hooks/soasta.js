...
{
	// MOD-2367: patch uncompressed zip data-descriptors
	var buffer = new Buffer(fs.readFileSync(inpath, {encoding: 'binary'}), 'binary'),
		patch = false;
	for (var i = 0; i <= buffer.length - 4; i++) {

		// found file entry
		if (buffer.readUInt32BE(i) === 0x504B0304) {

			// has data-descriptor and uncompressed?
			i += 6;
			if (buffer.readUInt16BE(i) === 0x800 && buffer.readUInt16BE(i + 2) === 0) {

				// remove data-descripor
				buffer.writeUInt16BE(0, i);
				patch = true;
	...
	if (patch) {
		fs.writeFileSync(inpath, buffer, {encoding: 'binary'});
	}
